...
public class MenuManager extends ContributionManager implements IMenuManager {
    ...
    public MenuManager(String text, ImageDescriptor image, String id) {
      ...
      ///############### START PATCH CONTRIBUTION ###############
        if (instanceCount++ == 0)
            System.out.println("Patched MenuManager class effective.");
      ///###############  END PATCH CONTRIBUTION  ###############
    }
    ...
    public void dispose() {
        ...
        ///############### START PATCH CONTRIBUTION ###############
        // Patch to clear up references to menu listeners when menu is
        // disposed. This was stopping views (among others) from being garbage
        // collected
        removeAll();
        listeners.clear();
        ///###############  END PATCH CONTRIBUTION  ###############
     ...
     * leaves out double separators and separators in the first or last
     * position.
     *
     * @param force
     ...
     * {@inheritDoc} <br>
     * <br> <b>
     * EUD Patch to filter the items contributed to the Menu because of: Eclipse Bug
     * 485931 - "Context sub-menu items appear twice when using
    ...
    protected boolean allowItem(IContributionItem itemToAdd)
    {
        ...
        /*
         * We will allow all the separators and group markers
         */
        if (itemToAdd instanceof AbstractGroupMarker)
        {
            return true;
        }
        /*
        ...
        for (IContributionItem currentItem : getItems())
        {
