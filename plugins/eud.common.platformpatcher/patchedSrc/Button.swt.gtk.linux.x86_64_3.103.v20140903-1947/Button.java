...
public Button (Composite parent, int style) {
    ...
    ///############### START PATCH CONTRIBUTION ###############    
    if (instanceCount++ == 0)
        System.out.println("Patched Button class effective.");
    ///###############  END PATCH CONTRIBUTION  ###############
...
public Point computeSize (int wHint, int hHint, boolean changed) {
    ...
    */
    ...
    if (wrap) {
        ...
        ///############### START PATCH CONTRIBUTION ###############
        // DM: push buttons have an extra padding area in GTK2, which make
        // toggle buttons smaller in comparison
        if (!OS.GTK3 && (style & (SWT.TOGGLE)) != 0)
        {
            size.x += 4;
            size.y += 4;
        }
...
void createHandle (int index) {
    ...
    ///############### START PATCH CONTRIBUTION ###############
    // DM: change button relief to avoid multiple selection lines in focused
    // buttons. GtkReliefStyle: GTK_RELIEF_NORMAL(0), GTK_RELIEF_HALF(1),
    // GTK_RELIEF_NONE
    else
    {
        OS._gtk_button_set_relief(handle, 1);
    }
    ///###############  END PATCH CONTRIBUTION  ###############
