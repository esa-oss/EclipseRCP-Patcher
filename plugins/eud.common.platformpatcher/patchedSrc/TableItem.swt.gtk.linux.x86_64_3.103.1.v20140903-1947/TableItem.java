...
TableItem (Table parent, int style, int index, boolean create) {
    ...
    ///############### START PATCH CONTRIBUTION ###############
    if (instanceCount++ == 0)
        System.out.println("Patched TableItem class effective.");
    ///###############  END PATCH CONTRIBUTION  ###############
...
public Rectangle getBounds () {
    ...
    ///############### START PATCH CONTRIBUTION ###############
    // DM: item bounds calculation is wrong for GTK2. See Eclipse Bug#512984
    //
    // Orig: return new Rectangle (rect.x, rect.y, width, rect.height + 1);
    //
    // Row height is increased by 3 (in relation to the original
    // implementation) because the method Rectangle#contains(Point)
    // considers the point located at (y + height) as out of the rectangle.
    // This causes that clicks on lines between rows are considered
    // "out of the table" and the column viewers return null rows.
    return OS.GTK3 ? new Rectangle (rect.x, rect.y, width, rect.height + 1) : new Rectangle(rect.x - 2, rect.y - 2, width + 2, rect.height + 4);
    ///###############  END PATCH CONTRIBUTION  ###############
}
...
public Rectangle getBounds (int index) {
    ...
    if (!OS.GTK3)
    {
        // Row height has to be increased by 3 (in relation to the original
        // implementation) because the method Rectangle#contains(Point)
        // considers the point located at (y + height) as out of the rectangle.
        // This causes that clicks on lines between rows are considered
        // "out of the table" and the column viewers return null rows.
        Rectangle ret = new Rectangle(rect.x - 2, rect.y - 2, width + 2, rect.height + 4);
    
        // DM: When a new table item is created in the lower area of the table,
        // the height returned is wrong. Take the first row as height reference
        if ((state & SKIN_NEEDED) != 0 && getParent().indexOf(this) != 0)
        {
            ret.height = getParent().getItem(0).getBounds(index).height;
        }
        return ret;
    }
    else
