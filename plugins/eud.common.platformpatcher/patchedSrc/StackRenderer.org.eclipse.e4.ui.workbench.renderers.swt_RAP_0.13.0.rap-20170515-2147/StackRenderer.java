...
public class StackRenderer extends LazyStackRenderer implements IPreferenceChangeListener {
     ...
     * @param ctf
     */
    private void addTopRight(final CTabFolder ctf) {
        final Composite trComp = new Composite(ctf, SWT.NONE);

        ///############### START PATCH CONTRIBUTION ###############
        // Remove RowLayout and place a listener to move the toolbar
        // below the tab header when necessary
        GridLayoutFactory.fillDefaults().numColumns(2).spacing(0, 0).applyTo(trComp);
        ///###############  END PATCH CONTRIBUTION  ###############

        ctf.setTopRight(trComp, SWT.FILL/* | SWT.WRAP */);
        
        // Initially it's not visible
        trComp.setVisible(false);

        // Create a TB for the view's drop-down menu
        final ToolBar menuTB = new ToolBar(trComp, SWT.FLAT | SWT.RIGHT);
        menuTB.setData(TAG_VIEW_MENU);
        GridData gd = new GridData();
        menuTB.setLayoutData(gd);
        ...
        ///############### START PATCH CONTRIBUTION ###############
        // Add a listener to wrap the cool bar
        addCoolBarResizeListener(ctf, trComp, menuTB);
        ///###############  END PATCH CONTRIBUTION  ###############
 ...
 && part.getToolbar().isToBeRendered();
            ...
            ///############### START PATCH CONTRIBUTION ###############
            //RowData is no longer used but GridData
            //RowData rd = (RowData) menuTB.getLayoutData();
            GridData gd = (GridData) menuTB.getLayoutData();
            if (needsMenu) {
...
//              menuTB.moveBelow(null);
                ...
                if (e.button == 1) {
                    ...
                    if (item != null) {
                        ...
                        if (ele.getParent().getSelectedElement() == ele) {
                            ...
                            ///############### START PATCH CONTRIBUTION ###############
                            Rectangle chevronRect = ctf.getAdapter( ICTabFolderAdapter.class ).getChevronRect();
                            if (chevronRect.contains(new Point(e.x, e.y)))
                            {
                                // Nothing to do. A setFocus operation would cause the 
                                // view list context menu to be closed before it is displayed
                                return;
                            }
                            ///###############  END PATCH CONTRIBUTION  ###############
 ...
 getMRUValueFromPreferences());        
        editorList.getTableViewer().getTable().setForeground(null);
        ///###############  END PATCH CONTRIBUTION  ###############
    ...
    ///############### START PATCH CONTRIBUTION ###############
    /**
     * Patch for the resizing of CTabFolder. It calculates the available space
     * and, if necessary, moves the CoolBar below the tab title area
     * 
     * @param ctf
     *            the reference to the CTabFolder
     * @param trComp
     *            the reference to the composite containing the CoolBar
     * @param menuTB
     *            the reference to the menu ToolBar
     */
    private void addCoolBarResizeListener(final CTabFolder ctf, final Composite trComp, final ToolBar menuTB)
    {
        trComp.addListener(SWT.Resize, (e) -> {
                // Remove listeners temporarily
                Listener[] tempListeners = trComp.getListeners(SWT.Resize);
                for (Listener l : tempListeners)
                {
                    trComp.removeListener(SWT.Resize, l);
                }
                Point topRightSize = menuTB
                        .computeSize(SWT.DEFAULT, SWT.DEFAULT);
                Point requiredSize = trComp.computeSize(SWT.DEFAULT,
                                                                 SWT.DEFAULT);  
                Rectangle titleArea = trComp.getBounds();
                boolean putTrimOnTop = (titleArea.width >= requiredSize.x && titleArea.height >= requiredSize.y);
                if (putTrimOnTop) {
                        Rectangle toolbarArea = new Rectangle(titleArea.x + titleArea.width - requiredSize.x, titleArea.y, requiredSize.x, titleArea.height);
                        
                        // Map the coordinates from the tabFolder back to the control's parent
                        Rectangle toolbarRect = trComp.getDisplay().map(ctf, trComp.getParent(), toolbarArea);
                        trComp.setBounds(toolbarRect);
                        
                        setIntegerFieldThroughReflection(ctf, "borderBottom", 0);
                } 
                else 
                {   
                    Rectangle ctfBounds = ctf.getBounds();
                    Rectangle toolbarArea = new Rectangle(ctfBounds.width - requiredSize.x - 8, 1, requiredSize.x, ctf.getTabHeight());
                    if (ctf.getSelection() != null)
                    {
                        Rectangle wrappedRegion = trComp.getDisplay().map(ctf.getSelection().getControl(), ctf, toolbarArea);
                        wrappedRegion.y = wrappedRegion.y - topRightSize.y - 2;
                        trComp.setBounds(wrappedRegion);
                            setIntegerFieldThroughReflection(ctf, "yClient", ctf.getTabHeight() + requiredSize.y + 5);
                            setIntegerFieldThroughReflection(ctf, "borderBottom", ctf.getTabHeight());
                    }
                }
                // Restore listeners
                for (Listener l : tempListeners)
                {
                    trComp.addListener(SWT.Resize, l);
                }
            });
    }
    
    // Patch - Private method to allow the set of an integer field through reflection
    private static void setIntegerFieldThroughReflection(Object object, String fieldToFind, int newValue)
    {
        Field field = null;
        // try to get the field reference from the class or the class
        // ancestors
        for (Class<?> c = object.getClass(); field == null && c != null; c = c.getSuperclass())
        {
            try
            {
                field = c.getDeclaredField(fieldToFind);
            }
            catch (NoSuchFieldException e)
            {
                // Do nothing, go to superclass
            }
        }
        if (field == null)
        {
        ...
        {
            throw new IllegalArgumentException("Field not accessible");
        }
